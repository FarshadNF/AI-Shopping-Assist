...
class ControllerExtensionModuleAiShoppingAssist extends Controller {
	...
	public function inject(&$route, &$data, &$output = null): void {
		...
		$asset_base = 'catalog/view/javascript/ai-shopping-assist/';
		...
		$config = [
			'chatRoute' => 'index.php?route=extension/module/ai_shopping_assist/chat',
			'catalogRoute' => 'index.php?route=extension/module/ai_shopping_assist/getCatalog',
			...
			'cartInfoRoute' => 'index.php?route=extension/module/ai_shopping_assist/getCart',
			'cartActionRoute' => 'index.php?route=extension/module/ai_shopping_assist/cartAction',
	...
	private function catalogAttributes(array $product): array {
		...
		if ($product_id && method_exists($this->model_catalog_product, 'getProductAttributes')) {
			try {
				foreach ($this->model_catalog_product->getProductAttributes($product_id) as $group) {
		...
		try {
			...
			foreach ($results as $result) {
				if ($this->config->get('total_' . $result['code'] . '_status')) {
					...
					$model = 'model_extension_total_' . $result['code'];
