(function () {
  class ManuAssistant {
    constructor(customConfig) {
      if (window.__MANU_LOADED__) return;
      window.__MANU_LOADED__ = true;
      ...
      this.defaults = {
        ...
        agentAvatars: {},
        agentProfiles: {
          manu: { name: 'Manu', role: 'Agent Manager', shortRole: 'Manager' },
      ...
      this.storagePrefix = 'manu:' + location.origin;
      this.keys = {
          ...
          <header class="aisa-head">
            <div class="aisa-head-main">
              ...
              <div class="aisa-identity">
                <span class="aisa-kicker">MANUAUTO SPECIALIST</span>
        ...
        <div class="aisa-launcher">
          ...
          <button type="button" class="aisa-toggle">
            ...
            <span class="aisa-toggle-copy">
              <strong>Manu</strong>
              <small>Industrial product desk</small>
            </span>
    ...
    cacheElements() {
      ...
      this.closedPromo = null;
      this.promoCharacter = null;
      this.promoOpenBtn = null;
      this.promoActionBtns = [];
      this.promoForm = null;
      ...
      this.toggleCopySmall = this.root.querySelector('.aisa-toggle-copy small');
    ...
    normalizeAgentKey(agentKey) {
      const normalizedKey = String(agentKey || 'manu').trim().toLowerCase();
      const agentAvatars = this.config.agentAvatars && typeof this.config.agentAvatars === 'object'
        ? this.config.agentAvatars
        : {};
      return Object.prototype.hasOwnProperty.call(agentAvatars, normalizedKey) ? normalizedKey : 'manu';
    }
    ...
    getActiveAgent() {
      try {
        return this.normalizeAgentKey(localStorage.getItem(this.getAgentStorageKey()) || 'manu');
      } catch (error) {
        return 'manu';
      }
    }
    ...
    getAgentProfile(agentKey) {
      ...
      const configured = profiles[normalizedKey] && typeof profiles[normalizedKey] === 'object'
        ...
        : {};
      const fallbackName = normalizedKey === 'manu'
    ...
    applyAgentAvatar(agentKey) {
      ...
      this.kicker.textContent = normalizedKey === 'manu'
        ? 'MANUAUTO SPECIALIST'
        : String(profile.role || 'Specialist Agent').toUpperCase();
      this.toggleText.textContent = 'Ask ' + profile.name;
      this.toggleText.dataset.role = profile.shortRole;
      if (this.toggleCopy) {
        this.toggleCopy.textContent = profile.name;
      }
      if (this.toggleCopySmall) {
        this.toggleCopySmall.textContent = normalizedKey === 'manu'
          ? 'Industrial product desk'
          : (profile.shortRole || profile.role || 'Specialist');
      }
      this.toggleBtn.setAttribute('aria-label', 'Chat with ' + profile.name);
      this.toggleBtn.setAttribute('title', 'Chat with ' + profile.name);
      this.input.placeholder = 'Ask ' + profile.name + '...';
    ...
    extractPageText() {
      ...
      clone.querySelectorAll(
        '#manu-widget, script, style, noscript, nav, header, footer, iframe, svg, .breadcrumb, .breadcrumbs, .aisa-panel'
    ...
    bindEvents() {
      ...
      if (this.promoForm && this.promoInput) {
      ...
      this.form.addEventListener('submit', async (event) => {
        ...
        this.input.value = '';
        if (typeof this.resizeInput === 'function') {
          this.resizeInput();
        }
    ...
    setStatus(text) {
      const label = this.statusLabel ? this.statusLabel.querySelector('span') : null;
      if (label) {
      ...
      this.root.classList.toggle('is-typing', /thinking|typing|working|loading/i.test(String(text || '')));
    }
    ...
    async handleAgentSwitchRequired(data, message) {
      ...
      const recommendedAgent = this.normalizeAgentKey(data.recommended_agent || 'manu');
      ...
      return new Promise((resolve) => {
        ...
        avatar.src = String((this.config.agentAvatars || {}).manu || this.config.avatarUrl || '');
        avatar.alt = 'Manu';
  ...
  function boot() {
    window.ManuAssistant = ManuAssistant;
    new ManuAssistant(window.MANU_CONFIG || {});
  }
