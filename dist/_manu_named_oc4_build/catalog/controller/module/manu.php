<?php
namespace Opencart\Catalog\Controller\Extension\Manu\Module;

class Manu extends \Opencart\System\Engine\Controller {
	private const VERSION = '4.2.12-manu';
	private const MARKER = '<!-- MANU_WIDGET -->';
	...
	private const AGENT_ROLES = [
		'manu' => 'Manu | Main orchestrator',
		'raya' => 'Mira | Pre-sales consultant',
		'scout' => 'Nexa | Product finder',
		'dex' => 'Volt | Technical specialist',
		'prism' => 'Vera | Comparison specialist',
		'atlas' => 'Sol | Solution consultant',
		'lia' => 'Scribe | Enquiry coordinator'
	];
	private const AGENT_DEFAULT_PROMPTS = [
		'manu' => 'Orchestrate the active specialists, resolve conflicts between their instructions, and deliver one coherent answer. Remain the only identity speaking to the customer.',
	...
	public function inject(&$route, &$data, &$output = null): void {
		...
		if (!$this->config->get('module_manu_status') || !$this->config->get('module_manu_footer_injection')) {
			return;
		}
		...
		$asset_base = 'extension/manu/catalog/view/javascript/manu/';
		...
		$config = [
			'apiBase' => '',
			'chatRoute' => 'index.php?route=extension/manu/module/manu.chat',
			'catalogRoute' => 'index.php?route=extension/manu/module/manu.getCatalog',
			...
			'cartInfoRoute' => 'index.php?route=extension/manu/module/manu.getCart',
			'cartActionRoute' => 'index.php?route=extension/manu/module/manu.cartAction',
			'checkoutRoute' => 'index.php?route=checkout/checkout',
			'couponRoute' => 'index.php?route=extension/opencart/total/coupon.save',
			'invoiceRoute' => 'index.php?route=extension/manu/module/manu.sendInvoice',
			'redirectLogRoute' => 'index.php?route=extension/manu/module/manu.logRedirect',
			'redirectUtm' => trim((string)$this->config->get('module_manu_redirect_utm')),
			'showNextQuestionSuggestions' => $this->suggestionSettingEnabled('module_manu_suggest_next_questions'),
			'showBlogSuggestions' => $this->suggestionSettingEnabled('module_manu_suggest_blogs'),
			'showCategorySuggestions' => $this->suggestionSettingEnabled('module_manu_suggest_categories'),
			'showProductSuggestions' => $this->suggestionSettingEnabled('module_manu_suggest_products'),
			'title' => $widget_title,
			...
			'characterUrl' => $asset_base . 'manu-character.png?v=' . self::VERSION,
			'agentAvatars' => [
				'manu' => $asset_base . 'avatars/manu.png?v=' . self::VERSION,
			...
			'agentProfiles' => [
				'manu' => ['name' => 'Manu', 'role' => 'Main Orchestrator', 'shortRole' => 'Specialist'],
				'raya' => ['name' => 'Mira', 'role' => 'Pre-sales Consultant', 'shortRole' => 'Pre-sales'],
				'scout' => ['name' => 'Nexa', 'role' => 'Product Finder', 'shortRole' => 'Product finder'],
				'dex' => ['name' => 'Volt', 'role' => 'Technical Specialist', 'shortRole' => 'Technical'],
				'prism' => ['name' => 'Vera', 'role' => 'Comparison Specialist', 'shortRole' => 'Comparison'],
				'atlas' => ['name' => 'Sol', 'role' => 'Solution Consultant', 'shortRole' => 'Solutions'],
				'lia' => ['name' => 'Scribe', 'role' => 'Enquiry Coordinator', 'shortRole' => 'Enquiries']
			],
		...
		$css_file = DIR_EXTENSION . 'manu/catalog/view/javascript/manu/widget.css';
		...
		$block = self::MARKER . "\n"
			. '<style id="manu-widget-styles">' . $widget_css . '</style>' . "\n"
			. '<script>window.MANU_CONFIG = Object.assign({}, window.MANU_CONFIG || {}, ' . $config_json . ');</script>' . "\n"
	...
	public function chat(): void {
		if (!$this->config->get('module_manu_status')) {
		...
		$current_agent_key = $this->normalizeAgentKey((string)($input['active_agent'] ?? 'manu'));
		...
		if ($switch_decision === 'approve' && $recommended_agent_key !== $current_agent_key) {
			...
			$active_agent_keys = $current_agent_key === 'manu' ? ['manu'] : ['manu', $current_agent_key];
		}

		$this->active_agent_key = (string)($active_agent_keys[1] ?? 'manu');
	...
	public function logRedirect(): void {
		...
		$utm_payload = substr(trim((string)($input['utm_payload'] ?? (string)$this->config->get('module_manu_redirect_utm'))), 0, 500);
	...
	private function askGemini(string $message, string $conversation_id, array $page_context = [], array $active_agent_keys = []): array {
		...
		$model = trim((string)$this->config->get('module_manu_gemini_model')) ?: 'gemini-2.5-flash';
		...
		$temperature = (float)($this->config->get('module_manu_gemini_temperature') ?: 0.3);
	...
	private function getGeminiApiKeys(): array {
		$raw = (string)$this->config->get('module_manu_gemini_api_key');
		$keys = preg_split('/[\r\n,]+/', $raw);
		$normalized = [];
	...
	private function buildGeminiPrompt(string $message, string $conversation_id, array $page_context = [], array $active_agent_keys = []): string {
		$brand = (string)($this->config->get('module_manu_store_brand') ?: $this->config->get('config_name'));
		$assistant_name = (string)($this->config->get('module_manu_assistant_name') ?: 'Manu');
		...
		$suggestion_controls = [
			'next_question_suggestions' => $this->suggestionSettingEnabled('module_manu_suggest_next_questions'),
			'blog_and_page_cards' => $this->suggestionSettingEnabled('module_manu_suggest_blogs'),
			'category_cards' => $this->suggestionSettingEnabled('module_manu_suggest_categories'),
			'product_cards' => $this->suggestionSettingEnabled('module_manu_suggest_products')
		];
	...
	private function configuredSitemapUrl(): string {
		$url = trim((string)$this->config->get('module_manu_sitemap_url'));
	...
	private function customSystemPrompt(): string {
		$value = str_replace(["\r\n", "\r"], "\n", trim((string)$this->config->get('module_manu_system_prompt')));
	...
	private function agentPrompt(string $agent_key): string {
		...
		$value = str_replace(
			["\r\n", "\r"],
			"\n",
			trim((string)$this->config->get('module_manu_prompt_' . $agent_key))
		);
	...
	private function normalizeAgentKey(string $agent_key): string {
		...
		return isset(self::AGENT_ROLES[$agent_key]) ? $agent_key : 'manu';
	}
	...
	private function getPromptCatalog(string $message): array {
		...
		$limit = (int)($this->config->get('module_manu_catalog_limit') ?: 80);
	...
	private function buildAssistantResponse(string $message, string $conversation_id, string $raw_text, array $page_context = []): array {
		...
		$suggestions = $this->suggestionSettingEnabled('module_manu_suggest_next_questions')
		...
		if (!$products && $this->suggestionSettingEnabled('module_manu_suggest_products')) {
	...
	private function filterSuggestionCards(array $cards): array {
		return array_values(array_filter($cards, function (array $card): bool {
			...
			if (in_array($content_type, ['blog', 'article', 'news', 'page'], true)) {
				return $this->suggestionSettingEnabled('module_manu_suggest_blogs');
			}

			if ($content_type === 'category') {
				return $this->suggestionSettingEnabled('module_manu_suggest_categories');
			}

			return $this->suggestionSettingEnabled('module_manu_suggest_products');
		}));
	}
