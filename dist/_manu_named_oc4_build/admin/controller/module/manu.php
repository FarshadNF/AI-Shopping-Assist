<?php
namespace Opencart\Admin\Controller\Extension\Manu\Module;

class Manu extends \Opencart\System\Engine\Controller {
	private const SETTING_CODE = 'module_manu';
	private const EVENT_CONTROLLER = 'manu_footer_controller';
	private const EVENT_VIEW = 'manu_footer_view';
	...
	public function index(): void {
		$this->load->language('extension/manu/module/manu');
		$this->createLogTable();
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/manu/module/manu', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/manu/module/manu.save', 'user_token=' . $this->session->data['user_token']);
		$data['clear_logs'] = $this->url->link('extension/manu/module/manu.clearLogs', 'user_token=' . $this->session->data['user_token']);
		$data['export_logs'] = $this->url->link('extension/manu/module/manu.exportLogs', 'user_token=' . $this->session->data['user_token']);
		$data['clear_redirect_logs'] = $this->url->link('extension/manu/module/manu.clearRedirectLogs', 'user_token=' . $this->session->data['user_token']);
		$data['export_redirect_logs'] = $this->url->link('extension/manu/module/manu.exportRedirectLogs', 'user_token=' . $this->session->data['user_token']);
		$data['clear_leads'] = $this->url->link('extension/manu/module/manu.clearLeads', 'user_token=' . $this->session->data['user_token']);
		$data['repair_lead_storage'] = $this->url->link('extension/manu/module/manu.repairLeadStorage', 'user_token=' . $this->session->data['user_token']);
		$data['export_leads'] = $this->url->link('extension/manu/module/manu.exportLeads', 'user_token=' . $this->session->data['user_token']);
		$data['upload_sitemap'] = $this->url->link('extension/manu/module/manu.uploadSitemap', 'user_token=' . $this->session->data['user_token']);
		...
		$data['module_manu_status'] = (int)$this->config->get('module_manu_status');
		$data['module_manu_gemini_api_key'] = (string)$this->config->get('module_manu_gemini_api_key');
		$data['module_manu_gemini_model'] = (string)($this->config->get('module_manu_gemini_model') ?: 'gemini-2.5-flash');
		$data['module_manu_gemini_temperature'] = (string)($this->config->get('module_manu_gemini_temperature') ?: '0.3');
		$data['module_manu_catalog_limit'] = (int)($this->config->get('module_manu_catalog_limit') ?: 80);
		$data['module_manu_store_brand'] = (string)($this->config->get('module_manu_store_brand') ?: $this->config->get('config_name'));
		$assistant_name = (string)$this->config->get('module_manu_assistant_name');
		$widget_title = (string)$this->config->get('module_manu_widget_title');
		$widget_button = (string)$this->config->get('module_manu_widget_button');
		...
		$data['module_manu_assistant_name'] = $assistant_name;
		$data['module_manu_sitemap_url'] = (string)$this->config->get('module_manu_sitemap_url');
		$data['module_manu_system_prompt'] = (string)$this->config->get('module_manu_system_prompt');
		$data['agent_prompts'] = [];

		foreach (self::AGENT_NAMES as $agent_key => $agent_name) {
			$setting_key = 'module_manu_prompt_' . $agent_key;
			$data['agent_prompts'][] = [
				'key' => $agent_key,
				'name' => $agent_name,
		...
		$data['module_manu_catalog_token'] = (string)$this->config->get('module_manu_catalog_token');
		$data['module_manu_footer_injection'] = $this->config->get('module_manu_footer_injection') !== null ? (int)$this->config->get('module_manu_footer_injection') : 1;
		$data['module_manu_suggest_next_questions'] = $this->config->get('module_manu_suggest_next_questions') !== null ? (int)$this->config->get('module_manu_suggest_next_questions') : 1;
		$data['module_manu_suggest_blogs'] = $this->config->get('module_manu_suggest_blogs') !== null ? (int)$this->config->get('module_manu_suggest_blogs') : 1;
		$data['module_manu_suggest_categories'] = $this->config->get('module_manu_suggest_categories') !== null ? (int)$this->config->get('module_manu_suggest_categories') : 1;
		$data['module_manu_suggest_products'] = $this->config->get('module_manu_suggest_products') !== null ? (int)$this->config->get('module_manu_suggest_products') : 1;
		$data['module_manu_widget_title'] = $widget_title;
		$data['module_manu_widget_button'] = $widget_button;
		$data['module_manu_redirect_utm'] = (string)$this->config->get('module_manu_redirect_utm');
		$data['user_token'] = $this->session->data['user_token'];
		$data['module_url'] = $this->url->link('extension/manu/module/manu', 'user_token=' . $data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/manu/module/manu', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/manu/module/manu')) {
			$json['error'] = $this->language->get('error_permission');
		}

		$status = (int)($this->request->post['module_manu_status'] ?? 0);
		$gemini_api_key = $this->normalizeApiKeys((string)($this->request->post['module_manu_gemini_api_key'] ?? ''));
		...
		if (!$json) {
			...
			$temperature = (float)($this->request->post['module_manu_gemini_temperature'] ?? 0.3);
			$temperature = min(1, max(0, $temperature));
			$catalog_limit = (int)($this->request->post['module_manu_catalog_limit'] ?? 80);
			$catalog_limit = min(200, max(5, $catalog_limit));

			$settings = [
				'module_manu_status' => $status,
				'module_manu_gemini_api_key' => $gemini_api_key,
				'module_manu_gemini_model' => trim((string)($this->request->post['module_manu_gemini_model'] ?? 'gemini-2.5-flash')) ?: 'gemini-2.5-flash',
				'module_manu_gemini_temperature' => (string)$temperature,
				'module_manu_catalog_limit' => $catalog_limit,
				'module_manu_store_brand' => trim((string)($this->request->post['module_manu_store_brand'] ?? $this->config->get('config_name'))) ?: $this->config->get('config_name'),
				'module_manu_assistant_name' => trim((string)($this->request->post['module_manu_assistant_name'] ?? 'Manu')) ?: 'Manu',
				'module_manu_sitemap_url' => $this->normalizeUrl((string)($this->request->post['module_manu_sitemap_url'] ?? '')),
				'module_manu_system_prompt' => $this->limitSettingText((string)($this->request->post['module_manu_system_prompt'] ?? ''), 12000),
				'module_manu_catalog_token' => trim((string)($this->request->post['module_manu_catalog_token'] ?? '')),
				'module_manu_footer_injection' => (int)($this->request->post['module_manu_footer_injection'] ?? 0),
				'module_manu_suggest_next_questions' => (int)($this->request->post['module_manu_suggest_next_questions'] ?? 0),
				'module_manu_suggest_blogs' => (int)($this->request->post['module_manu_suggest_blogs'] ?? 0),
				'module_manu_suggest_categories' => (int)($this->request->post['module_manu_suggest_categories'] ?? 0),
				'module_manu_suggest_products' => (int)($this->request->post['module_manu_suggest_products'] ?? 0),
				'module_manu_widget_title' => trim((string)($this->request->post['module_manu_widget_title'] ?? 'Manu')),
				'module_manu_widget_button' => trim((string)($this->request->post['module_manu_widget_button'] ?? 'Chat')),
				'module_manu_redirect_utm' => $this->limitSettingText((string)($this->request->post['module_manu_redirect_utm'] ?? ''), 500)
			];
		...
		if (!$this->isAjaxRequest()) {
			...
			$redirect = html_entity_decode($this->url->link('extension/manu/module/manu', 'user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
	...
	public function exportLogs(): void {
		...
		if (!$this->user->hasPermission('access', 'extension/manu/module/manu')) {
	...
	public function install(): void {
		...
		$this->load->model('setting/setting');
		$settings = [
			'module_manu_status' => 0,
			'module_manu_gemini_api_key' => '',
			'module_manu_gemini_model' => 'gemini-2.5-flash',
			'module_manu_gemini_temperature' => '0.3',
			'module_manu_catalog_limit' => 80,
			'module_manu_store_brand' => $this->config->get('config_name'),
			'module_manu_assistant_name' => 'Manu',
			'module_manu_sitemap_url' => '',
			'module_manu_system_prompt' => '',
			'module_manu_catalog_token' => '',
			'module_manu_footer_injection' => 1,
			'module_manu_suggest_next_questions' => 1,
			'module_manu_suggest_blogs' => 1,
			'module_manu_suggest_categories' => 1,
			'module_manu_suggest_products' => 1,
			'module_manu_widget_title' => 'Manu',
			'module_manu_widget_button' => 'Chat',
			'module_manu_redirect_utm' => 'utm_source=manu&utm_medium=assistant&utm_campaign=chat'
		];

		foreach (self::AGENT_NAMES as $agent_key => $agent_name) {
			$settings['module_manu_prompt_' . $agent_key] = '';
		}
		...
		$this->model_setting_event->addEvent([
			'description' => 'Manu - inject widget after footer controller',
			'code' => self::EVENT_CONTROLLER,
			'trigger' => 'catalog/controller/common/footer/after',
			'action' => 'extension/manu/module/manu.inject',
			'status' => 1,
			'sort_order' => 10
		]);

		$this->model_setting_event->addEvent([
			'description' => 'Manu - inject widget after footer view',
		...
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/manu/module/manu');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/manu/module/manu');
	}
	...
	private function buildPagination(int $total, int $page, int $limit, string $page_key, array $extra = []): string {
		...
		return $this->load->controller('common/pagination', [
			'total' => max(0, $total),
			'page' => max(1, $page),
			'limit' => max(1, $limit),
			'url' => $this->url->link('extension/manu/module/manu', implode('&', $params))
		]);
	}
	...
	private function uploadedSitemapCachePaths(): array {
		...
		$paths[] = rtrim(DIR_CACHE, '/\\') . DIRECTORY_SEPARATOR . 'cache.manu.sitemap.upload.xml';

		if (defined('DIR_STORAGE')) {
			$paths[] = rtrim((string)DIR_STORAGE, '/\\') . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'cache.manu.sitemap.upload.xml';
		}

		if (defined('DIR_SYSTEM')) {
			$paths[] = rtrim(dirname(rtrim((string)DIR_SYSTEM, '/\\')), '/\\') . DIRECTORY_SEPARATOR . 'system' . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'cache.manu.sitemap.upload.xml';
		}

		if (defined('DIR_APPLICATION')) {
			$paths[] = rtrim(dirname(rtrim((string)DIR_APPLICATION, '/\\')), '/\\') . DIRECTORY_SEPARATOR . 'system' . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'cache.manu.sitemap.upload.xml';
		}
	...
	private function clearSitemapCaches(): void {
		...
		$patterns = [
			'cache.manu.sitemap.v*.json',
			'cache.manu.sitemap.upload.v*.json',
			'cache.manu.sitemap.content.v1.*.json',
			'cache.manu.sitemap.content.v2.*.json'
		];
	...
	private function sitemapUrlCandidates(): array {
		$urls = [];
		$configured = trim((string)$this->config->get('module_manu_sitemap_url'));
	...
	private function findSitemapContentCachePath(): string {
		...
		foreach (['cache.manu.sitemap.content.v2.*.json', 'cache.manu.sitemap.content.v1.*.json'] as $pattern) {
	...
	private function getWarmSitemapUrl(): string {
		...
		$query = 'route=extension/manu/module/manu.warmSitemap&limit=120&refresh=1';
		$token = trim((string)$this->config->get('module_manu_catalog_token'));
