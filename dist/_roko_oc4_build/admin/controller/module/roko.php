<?php
namespace Opencart\Admin\Controller\Extension\Roko\Module;

class Roko extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		$this->load->language('extension/roko/module/roko');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/roko/module/roko', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/roko/module/roko.save', 'user_token=' . $this->session->data['user_token']);
		$data['clear_logs'] = $this->url->link('extension/roko/module/roko.clearLogs', 'user_token=' . $this->session->data['user_token']);
		$data['export_logs'] = $this->url->link('extension/roko/module/roko.exportLogs', 'user_token=' . $this->session->data['user_token']);
		$data['import_knowledge'] = $this->url->link('extension/roko/module/roko.importKnowledge', 'user_token=' . $this->session->data['user_token']);
		$data['clear_knowledge'] = $this->url->link('extension/roko/module/roko.clearKnowledge', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/roko/module/roko', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/roko/module/roko')) {
		...
		if (!$json) {
			...
			$settings = [
				...
				'module_roko_widget_button' => trim((string)($this->request->post['module_roko_widget_button'] ?? 'Ask ROKO'))
			];
		...
		if (!$json) {
			...
			if (!is_array($payload) || json_last_error() !== JSON_ERROR_NONE) {
				...
				try {
					...
					$this->log->write('ROKO knowledge import failed: ' . $exception->getMessage());
	...
	public function clearKnowledge(): void {
		...
		if (!$json) {
			try {
				$this->db->query('DELETE FROM `' . DB_PREFIX . 'roko_knowledge`');
	...
	public function exportLogs(): void {
		...
		if (!$this->user->hasPermission('access', 'extension/roko/module/roko')) {
	...
	public function install(): void {
		...
		$this->model_setting_setting->editSetting(self::SETTING_CODE, [
			...
			'module_roko_widget_button' => 'Ask ROKO'
		]);
		...
		$this->model_setting_event->addEvent([
			'description' => 'ROKO - inject widget after footer controller',
			'code' => self::EVENT_CONTROLLER,
			'trigger' => 'catalog/controller/common/footer/after',
			'action' => 'extension/roko/module/roko.inject',
			'status' => 1,
			'sort_order' => 10
		]);

		$this->model_setting_event->addEvent([
			'description' => 'ROKO - inject widget after footer view',
	...
	private function createKnowledgeTable(): void {
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "roko_knowledge` (
	...
	private function getKnowledgeCount(): int {
		try {
			$query = $this->db->query('SELECT COUNT(*) AS `total` FROM `' . DB_PREFIX . 'roko_knowledge`');
	...
	private function importBundledKnowledge(): void {
		...
		if (defined('DIR_EXTENSION')) {
			array_unshift($candidates, rtrim(DIR_EXTENSION, '/\\') . '/roko/install_data/enriched_cache.json');
		}
		...
		foreach ($candidates as $path) {
			...
			try {
				...
				$this->log->write('ROKO bundled knowledge import failed: ' . $exception->getMessage());
			}
	...
	private function replaceKnowledge(array $payload): int {
		...
		try {
			...
			foreach ($rows as $row) {
				$this->db->query("
					INSERT INTO `" . DB_PREFIX . "roko_knowledge`
					SET
