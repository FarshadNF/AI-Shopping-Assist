(function () {
  class RokoAssistant {
    ...
    createProductCard(product) {
      ...
      card.className = 'aisa-product-card ' + (isProduct ? 'is-product' : 'is-article');

      const row = document.createElement('div');
      row.className = 'aisa-product-row';
      ...
      if (product.image) {
        ...
        placeholder.textContent = isProduct ? 'P' : 'A';
      ...
      if (isProduct && product.product_id && product.stock !== 0) {
        const add = document.createElement('button');
        add.type = 'button';
        add.className = 'aisa-product-action secondary';
